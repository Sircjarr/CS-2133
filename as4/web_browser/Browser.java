// Browser assumes user will always enter 'http://' before the url

public class Browser {
	
	public static BFrame bf;
	
	public static void main(String[] args) {
		bf = new BFrame();
		/*A socket is one endpoint of a two-way communication link between two programs running
		on the network. A socket is bound to a port number so that the TCP layer can identify 
		the application that data is destined to be sent to. An endpoint is a combination of 
		an IP address and a port number*/
	}
}